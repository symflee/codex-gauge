...
앱을 처음 실행하면 macOS의 선호 언어 목록에서 한국어와 영어 중 먼저 발견한 언어를 초기 UI 언어로 한 번 선택해 저장합니다. 지원 언어가 없으면 영어를 사용하며, 이후에는 앱의 `설정…`에서 `한국어` 또는 `English`를 선택할 수 있습니다. 변경은 상태 항목의 VoiceOver 문구, 메뉴와 열린 설정 창에 즉시 적용됩니다.
...
- 설정에는 UI 언어, 표시 방식, 갱신 프리셋, 선택한 실행 파일 경로 같은 비밀이 아닌 값만 저장합니다.
