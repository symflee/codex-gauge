...
`v0.1.0` 사용자는 앱을 종료하고 `v0.2.0` DMG로 기존 앱을 한 번 교체해야 합니다. `v0.2.0`부터는 앱 시작 시 새 버전을 한 번 확인하며, 새 버전이 있으면 버전 메뉴에서 업데이트를 시작할 수 있습니다. 다운로드와 설치는 사용자가 선택한 경우에만 진행합니다.

## 사용

메뉴 막대 캡슐에서 Codex 한도 하나의 남은 비율을 확인합니다. 설정에서 표시할 기간을 선택하거나 자동 선택을 사용하고, 게이지 색상을 바꿀 수 있습니다. 상태 항목을 누르면 모든 기간의 상세 정보를 볼 수 있습니다. 값을 알 수 없는 상태를 남은 한도 `0%`로 표시하지 않습니다. Codex 버전이나 연결 상태에 따라 조회가 일시적으로 실패할 수 있습니다.
